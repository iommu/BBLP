...
void ESP32Encoder::attach(int a, int b, enum encType et) {
  ...
  r_enc_config.pos_mode =
      fullQuad ? PCNT_COUNT_DEC : PCNT_COUNT_DIS; // Count Only On Rising-Edges
  ...
  if (attachedInterrupt == false) {
    ...
    esp_err_t er =
        pcnt_isr_register(pcnt_example_intr_handler, (void *)NULL, (int)0,
...
int64_t ESP32Encoder::getCount() { return getCountRaw() + count; }
